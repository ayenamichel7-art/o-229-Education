...
export const Students: React.FC = () => {
  ...
  const [bulletinStudent, setBulletinStudent] = useState<{
    id: number;
    name: string;
  } | null>(null);
  ...
  const isManager = user?.roles?.some((r) =>
    ["super-admin", "admin-school", "director"].includes(r),
  );
  ...
  const handleDelete = async (id: number) => {
    if (
      ...
      const loadingToast = toast.loading(
        t("common.loading") || "Suppression en cours...",
      );
      try {
      ...
      <div className="page-header">
        <div>
          ...
          <p style={{ color: "var(--color-text-light)", marginTop: 4 }}>
            {t("students.registered_count", { count: filtered.length })}
          </p>
        </div>
        {isManager && (
          <button
                      ...
                      style={{
                        background:
                          student.status === "active"
                            ...
                            : "rgba(239,68,68,0.1)",
                        color:
                          student.status === "active" ? "#059669" : "#dc2626",
                        padding: "4px 12px",
                        borderRadius: 20,
                        fontSize: "0.8rem",
                        fontWeight: 500,
