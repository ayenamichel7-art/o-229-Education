...
export const Finance: React.FC = () => {
  ...
  const { user } = useAuth();
  const { payments, summary, isLoading, fetchPayments, fetchSummary } =
    useFinanceStore();
  const [isModalOpen, setIsModalOpen] = useState(false);

  const isManager = user?.roles?.some((r) =>
    ["super-admin", "admin-school", "director", "accountant"].includes(r),
  );
  ...
  const formatCFA = (n: number | undefined | null) =>
        ...
        {[
          {
            ...
            value: formatCFA(summary?.total_collected),
          ...
          {
            label: t("common.pending"),
            value: formatCFA(summary?.total_pending),
            icon: <Clock size={22} />,
            color: "#f59e0b",
            trend: "-3%",
          ...
          {
            label: t("common.overdue"),
            value: formatCFA(summary?.total_overdue),
              ...
              {payments.map((p) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {new Date(
                        p.created_at || p.due_date || new Date(),
