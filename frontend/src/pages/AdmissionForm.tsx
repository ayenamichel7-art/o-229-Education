    ...
    const fetchTemplate = async () => {
      try {
        ...
        setTemplate({
          ...
          description:
            "Veuillez remplir ce formulaire pour soumettre la candidature de votre enfant.",
          fields: [
            {
              id: 1,
              name: "first_name",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.post(
        `/form-templates/${template!.id}/submit`,
        {
          fields: formData,
        },
      );
      ...
      if (requires_payment && payment_url) {
        setSuccessMsg(
          "Candidature soumise ! Vous allez être redirigé vers le paiement des frais d'inscription...",
        );
        setTimeout(() => {
        ...
        setSuccessMsg(
          "Votre dossier a été soumis avec succès ! Nous vous contacterons sous peu.",
        );
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "800px", margin: "0 auto", background: "white" }}
      >
        ...
        >
          {template.fields.map((field) => (
            ...
            >
              ...
              {["text", "email", "number", "date", "file"].includes(
                field.type,
              ) && (
                <input
                  type={field.type}
